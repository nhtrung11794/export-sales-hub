...
export default function M03Page() {
  ...
  const [isValid, setIsValid] = useState(true); // Dummy validation for M03 draft
  ...
  const handleSubmit = async () => {
    ...
    if (result.success) alert('Nộp bài thành công! Module 04 đã được mở khóa.');
  ...
  const dynamicPrompt = `"Đóng vai một chuyên gia B2B Sales, hãy gợi ý cho tôi 3 phương pháp tiếp cận khách hàng tiềm năng hiệu quả nhất."`;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div className="glass-panel" style={{ padding: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          >
            <BookOpen size={16}/> {loadingFile === 'M03_Bai05.pdf' ? 'Đang tải...' : 'Giáo án PDF'}
          </button>
          <button 
            ...
            style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '0.85rem' }}
          >
            <Play size={16}/> {loadingVideo === 'M03_Video05.mp4' ? 'Đang tải...' : 'Video Tổng kết'}
          </button>
        </div>
      </div>
  ...
  const aiTutorContent = (
    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-5 rounded-xl border border-emerald-100 shadow-sm">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-base font-bold text-emerald-900">AI Prospecting Tutor</h3>
      </div>
      <p className="text-sm text-emerald-800/80 mb-5 leading-relaxed">
        Gặp khó khăn khi viết email tiếp cận khách hàng? Hãy để AI gợi ý kịch bản mở lời (Cold Email).
      </p>
      <button
        onClick={handleCopyPrompt}
        className="w-full relative group overflow-hidden bg-emerald-600 hover:bg-emerald-700 text-white font-medium text-sm py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all shadow-sm hover:shadow"
      >
